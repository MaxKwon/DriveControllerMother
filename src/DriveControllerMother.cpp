...
const double K_P_YAW_T = 0.0; //TeleopDrive
...
DriveControllerMother::DriveControllerMother(int fl, int fr, int rl, int rr,
	...
	canTalonRearLeft = new CANTalon(RL);
	canTalonKicker = new CANTalon(K);
...
void DriveControllerMother::Controller(double ref_kick, double ref_right,
	...
	canTalonRearRight->Set(total_right);
	canTalonRearLeft->Set(-total_left);
...
void DriveControllerMother::StopAll() {
	...
	canTalonFrontLeft->Set(0.0); //0.0 because of the double, 0 should still work but better safe than sorry
	canTalonFrontRight->Set(0.0);
	canTalonRearRight->Set(0.0);
	canTalonRearLeft->Set(0.0);
	canTalonKicker->Set(0.0);
...
void DriveControllerMother::ZeroEncs() {
	...
	canTalonRearRight->SetEncPosition(0);
	canTalonRearLeft->SetEncPosition(0);
